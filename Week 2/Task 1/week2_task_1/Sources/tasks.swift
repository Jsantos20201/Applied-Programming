...
func task0(aBool: Bool, int1: Int, int2: Int, aString: String) -> GuardedErrors? {
    ...
    guard int1 >= 0 else {
        testPrint("Error: \(GuardedErrors.int1_less_than_0.rawValue)")
...
func task1(_ optionalString: String?) -> GuardedErrors? {
    guard let str = optionalString else {
        return .string_is_nil
    }
    
    testPrint("Type \"\(str)\" into the console:")
    
    guard let input = testReadLine() else {
        return .testReadLine_is_nil
    }
    
    guard input == str else {
        return .input_does_not_match
    }
...
func task2() -> Int? {
    ...
    while true {
        testPrint("Please enter your name:")
        if let name = testReadLine(), !name.isEmpty {
            if name == "done" {
                break
            }
            testPrint("Hello \(name)!")
            count += 1
        }
    }
     ...
     let closureArray: [task3Func] = [
        ...
        { // Closure 3:
            ...
            if first_parameter < 0 {
                return -1
            } else if first_parameter >= test3Array.count {
                return -2
            } else if test3Array[first_parameter] != second_parameter {
                return 0
            } else {
                return 1
            }
        ...
        { // Closure 4:
            first_parameter, second_parameter in
            if first_parameter % 2 != 0 && second_parameter % 2 != 0 {
                return -1
            } else if first_parameter % 2 == 0 && second_parameter % 2 == 0 {
                return 1
            } else {
                return 0
            }
...
func task7(inArray: [Int]) -> [Int]? {
    ...
    // add the numbers from 2 through 7
    for i in 2...7 {
        returnArray.append(i)
    }
    
    // add the odd numbers from 1 through 9
    for i in stride(from: 1, to: 10, by: 2) {
        returnArray.append(i)
    }
    
    // add the numbers from 5 through 0
    for i in stride(from: 5, through: 0, by: -1) {
        returnArray.append(i)
    }
    
    // add the contents of the input array
    for num in inArray {
...
func task8(rowCount: Int, columnCount: Int, aRange: ClosedRange<Int>) -> [[Int]]? {
    ...
    for _ in 1...rowCount {
        ...
        for _ in 1...columnCount {
            let randomInt = Int.random(in: aRange)
